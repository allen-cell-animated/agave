...
class DummyClient(WebSocketClient):
    def opened(self):
        ...
        cb = CommandBuffer()
        cb.add_command("LOAD_OME_TIF", "C:\\Users\\danielt.ALLENINST\\Downloads\\AICS-12_57_7.ome.tif")
        buf = cb.make_buffer()
        self.send(buf, True)
        ...
        for i in range(0, rot):
            ...
            cb.add_command("MAT_DIFFUSE", 0, (float(i))/float(rot-1), (float(rot-1)-float(i))/float(rot-1), 0.0, 1.0)
            cb.add_command("MAT_SPECULAR", 0, 3.0, 3.0, 3.0, 0.0)
            cb.add_command("MAT_EMISSIVE", 0, 0.0, 0.0, 0.0, 0.0)
            cb.add_command("MAT_DIFFUSE", 1, (float(i))/float(rot-1), (float(rot-1)-float(i))/float(rot-1), 0.0, 1.0)
            cb.add_command("MAT_SPECULAR", 1, 3.0, 3.0, 3.0, 0.0)
            cb.add_command("MAT_EMISSIVE", 1, 0.0, 0.0, 0.0, 0.0)
            cb.add_command("MAT_DIFFUSE", 2, (float(i))/float(rot-1), (float(rot-1)-float(i))/float(rot-1), 0.0, 1.0)
            cb.add_command("MAT_SPECULAR", 2, 3.0, 3.0, 3.0, 0.0)
            cb.add_command("MAT_EMISSIVE", 2, 0.0, 0.0, 0.0, 0.0)
