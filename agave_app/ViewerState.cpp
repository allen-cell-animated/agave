...
stateToPythonScript(const Serialize::ViewerState& s)
{
  ...
  // For now, most users will probably not mind that we are putting the executable
  // path in the Python script.
  // Pass auto_launch=False or omit agave_path to customize this behavior.
  ...
  ss << obj << SetMultichannelBlendCommand({ s.pathTracer.multichannelBlendMode }).toPythonString() << std::endl;
