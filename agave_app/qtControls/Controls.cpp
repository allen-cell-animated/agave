...
QColorWithIntensity::QColorWithIntensity(QColor color, float intensity, QWidget* parent)
  : QWidget(parent)
{
  auto* layout = new QHBoxLayout();
  m_intensitySlider = new QNumericSlider();
  m_intensitySlider->setStatusTip(tr("Set intensity"));
  m_intensitySlider->setToolTip(tr("Set intensity"));
  m_intensitySlider->setRange(0.0, 1000.0);
  m_intensitySlider->setSingleStep(10.0);
  m_intensitySlider->setValue(intensity);
  m_intensitySlider->setDecimals(1);
  layout->addWidget(m_intensitySlider, 1);
  m_colorButton = new QColorPushButton();
  m_colorButton->setStatusTip(tr("Set color"));
  m_colorButton->setToolTip(tr("Set color"));
  m_colorButton->SetColor(color);
  layout->addWidget(m_colorButton, 0);
  layout->setContentsMargins(0, 0, 0, 0);
  setLayout(layout);

  // connect intensity slider to emit intensityChanged
  connect(m_intensitySlider, &QNumericSlider::valueChanged, this, &QColorWithIntensity::intensityChanged);
  connect(m_colorButton, &QColorPushButton::currentColorChanged, this, &QColorWithIntensity::colorChanged);
}

void
QColorWithIntensity::setIntensity(float intensity)
{
  m_intensitySlider->setValue(intensity);
}

void
QColorWithIntensity::setColor(const QColor& color)
{
  m_colorButton->SetColor(color);
}

QColor
QColorWithIntensity::getColor()
{
  return m_colorButton->GetColor();
}

float
QColorWithIntensity::getIntensity()
{
