...
void
RenderThread::configure(IRenderWindow* renderer,
...
RenderThread::run()
{
  m_thread = std::thread(&RenderThread::mainThreadFunc, this);
}
...
RenderThread::mainThreadFunc(RenderThread* rt)
{
  ...
  while (!rt->isInterruptionRequested()) {
    rt->processRequest();
  ...
  rt->m_rglContext.makeCurrent();
  if (rt->m_myVolumeData.ownRenderer) {
