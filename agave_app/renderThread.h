...
public:
  ...
  // Call from mainthread to set up
  // DANGER
  // we are going to be reusing resources from the main thread here.
  // so we need to make sure that the main thread is not using them!!!
  ...
  // this will start the thread and run the render loop.
  void run();

private:
  std::thread m_thread;

  static void mainThreadFunc(RenderThread* rt);
};
