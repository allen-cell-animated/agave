...
static const char* vertex_shader_text_no_miter =
  R"(
    #version 400 core
...
//};
    ...
    vec4 va[2];
    // put everything in pixel space so we can apply thickness
    for (int i=0; i<2; ++i)
    {
        vec4 vtx;
        vtx.x = texelFetch(stripVerts, (stripVertexOffset + line_i+i)*10 + 0).x;
    ...
    if (tri_i == 0 || tri_i == 1 || tri_i == 3)
    {
        ...
        vUV.x = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 3).x;
        vUV.y = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 4).x;

        vCol.x = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 5).x;
        vCol.y = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 6).x;
        vCol.z = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 7).x;
        vCol.w = texelFetch(stripVerts, (stripVertexOffset + line_i)*10 + 8).x;
