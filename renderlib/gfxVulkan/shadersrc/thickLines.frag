...
layout(set = 0, binding = 0, std140) uniform ThickLinesFragParams
{
  int picking; //< draw for display or for picking? Picking has no texture.
}
thickLinesFragParams;
...
main()
{
  ...
  if (thickLinesFragParams.picking == 1) {
    result.w = 1.0;
  }
  ...
  if (thickLinesFragParams.picking == 0 && Frag_UV.x > -64 + EPSILON) {
