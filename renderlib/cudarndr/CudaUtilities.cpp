...
void HandleCudaError(const cudaError_t CudaError, const char* pDescription /*= ""*/)
{
	...
	LOG_ERROR << "Encountered a critical CUDA error: " << std::string(pDescription) << " " << std::string(cudaGetErrorString(CudaError));

	throw std::string("Encountered a critical CUDA error: ") + std::string(pDescription) + std::string(" ") + std::string(cudaGetErrorString(CudaError));
}
