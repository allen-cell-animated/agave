...
void
CVariance::Resize(int Width, int Height, CStatus& status)
{
  if (Width == 0 || Height == 0)
    return;
  ...
  status.SetStatisticChanged(
    "Variance", "N buffer", QString::number((float)NoElements * sizeof(int) / MB, 'f', 2), "MB");

  cudaMalloc((void**)&m_pOldM, NoElements * sizeof(float));
  status.SetStatisticChanged(
    "Variance", "Old Mean Buffer", QString::number((float)NoElements * sizeof(float) / MB, 'f', 2), "MB");

  cudaMalloc((void**)&m_pNewM, NoElements * sizeof(float));
  status.SetStatisticChanged(
    "Variance", "New Mean Buffer", QString::number((float)NoElements * sizeof(float) / MB, 'f', 2), "MB");

  cudaMalloc((void**)&m_pOldS, NoElements * sizeof(float));
  status.SetStatisticChanged(
    "Variance", "Old S Buffer", QString::number((float)NoElements * sizeof(float) / MB, 'f', 2), "MB");

  cudaMalloc((void**)&m_pNewS, NoElements * sizeof(float));
  status.SetStatisticChanged(
    "Variance", "New S Buffer", QString::number((float)NoElements * sizeof(float) / MB, 'f', 2), "MB");

  cudaMalloc((void**)&m_pVariance, NoElements * sizeof(float));
  status.SetStatisticChanged(
    "Variance", "Magnitude Buffer", QString::number((float)NoElements * sizeof(float) / MB, 'f', 2), "MB");

  Reset();
}

void
CVariance::Reset(void)
{
  const int NoElements = m_Width * m_Height;
...
float*
CVariance::GetVarianceBuffer(void)
{
  return m_pVariance;
}

float
CVariance::GetMeanVariance(void) const
{
  return m_MeanVariance;
}

void
CVariance::SetMeanVariance(const float& Variance)
{
