...
DEV float
vecLen(float4 a, float4 b)
{
  return ((b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y) + (b.z - a.z) * (b.z - a.z));
}

DEV inline void
XYZToRGB(const float4& xyz, float4& rgb)
{
...
KERNEL void
KrnlDenoise(float* inbuf, cudaSurfaceObject_t surfaceObj)
{
  ...
  uchar4 pixel = make_uchar4(
    __saturatef(clr.x) * 255.0, __saturatef(clr.y) * 255.0, __saturatef(clr.z) * 255.0, __saturatef(clr00.w) * 255.0);
  surf2Dwrite(pixel, surfaceObj, X * 4, Y);
  //	}
  //	else
  //	{
  //		// blit the tonemapped result.  this is nonsensical.  the conditional should be before invoking these
  // kernels.
