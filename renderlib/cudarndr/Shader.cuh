...
{
public:
  HOD CLambertian(const CColorXyz& Kd) { m_Kd = Kd; }

  HOD ~CLambertian(void) {}

  HOD CColorXyz F(const float3& Wo, const float3& Wi) { return m_Kd * INV_PI_F; }
...
FrDiel(float cosi, float cost, const CColorXyz& etai, const CColorXyz& etat)
{
...
public:
  ...
  HOD float Pdf(const float3& Wo, const float3& Wi) { return INV_4_PI_F; }
...
public:
  ...
  HOD float3 WorldToLocal(const float3& W) { return make_float3(dot(W, m_Nu), dot(W, m_Nv), dot(W, m_Nn)); }
...
public:
  ...
  HOD CVolumeShader(const EType& Type,
                    const float3& N,
                    const float3& Wo,
                    const CColorXyz& Kd,
                    const CColorXyz& Ks,
