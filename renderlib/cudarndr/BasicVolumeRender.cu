...
DEV float3
mul(const float3x4& M, const float3& v)
{
  float3 r;
...
DEV float4
mul(const float3x4& M, const float4& v)
{
  float4 r;
  r.x = dot(v, M.m[0]);
  r.y = dot(v, M.m[1]);
...
// transform vector by matrix (no translation)
DEV float3
mul(const float4x4& M, const float3& v)
{
  float3 r;
  r.x = dot(v, make_float3(M.m[0]));
  r.y = dot(v, make_float3(M.m[1]));
...
// transform vector by matrix with translation
DEV float4
mul(const float4x4& M, const float4& v)
{
  float4 r;
  r.x = dot(v, M.m[0]);
  r.y = dot(v, M.m[1]);
...
DEV int
intersectBox(Ray r, float3 boxmin, float3 boxmax, float* tnear, float* tfar)
{
...
KERNEL void
KrnlCh(cudaTextureObject_t volumeTex,
       float* outbuf,
       float density,
       float brightness,
       float dataRangeMin,
       float dataRangeMax)
{
  ...
  for (int i = 0; i < maxSteps; i++) {
    ...
    float4 col =
      make_float4(sample, sample, sample, sample); // tex1D(transferTex, (sample - transferOffset)*transferScale);
    col.w *= density;
...
void
RayMarchVolume(float* outbuf,
               cudaTextureObject_t volumeTex,
               cudaTextureObject_t gradientVolumeTex,
               int w,
               int h,
               float density,
               float brightness,
...
KERNEL void
KrnlToneMap_Basic(float* inbuf, cudaSurfaceObject_t surfaceObj)
{
  const int X = blockIdx.x * blockDim.x + threadIdx.x;
  const int Y = blockIdx.y * blockDim.y + threadIdx.y;
...
void
ToneMap_Basic(float* inbuf, cudaSurfaceObject_t surfaceObj, int w, int h)
{
  // init some input vars for kernel
