...
DEV inline bool SampleDistanceRM(CRay& R, CRNG& RNG, Vec3f& Ps, const cudaVolume& volumedata)
{
	...
	while (Sum < S)
	{
		...
		SigmaT	= gDensityScale * GetOpacity(GetNormalizedIntensity(Ps, volumedata.volumeTexture));
...
DEV inline bool FreePathRM(CRay& R, CRNG& RNG, const cudaVolume& volumedata)
{
	...
	while (Sum < S)
	{
		...
		SigmaT	= gDensityScale * GetOpacity(GetNormalizedIntensity(Ps[TID], volumedata.volumeTexture));
...
DEV inline bool NearestIntersection(CRay R, CScene* pScene, const cudaVolume& volumedata, float& T)
{
	...
	while (T < MaxT)
	{
		...
		if (GetOpacity(GetNormalizedIntensity(Ps, volumedata.volumeTexture)) > 0.0f)
			return true;
