...
void ChannelCuda::allocGpu(ImageXYZC* img, int channel, bool do_volume, bool do_gradient_volume) {
	...
	if (do_volume) {
		...
		m_gpuBytes += (channelDesc.x + channelDesc.y + channelDesc.z + channelDesc.w)/8 * volumeSize.width * volumeSize.height * volumeSize.depth;
	...
	if (do_gradient_volume) {
		...
		m_gpuBytes += (gradientChannelDesc.x + gradientChannelDesc.y + gradientChannelDesc.z + gradientChannelDesc.w) / 8 * volumeSize.width * volumeSize.height * volumeSize.depth;
...
void ChannelCuda::updateLutGpu(int channel, ImageXYZC* img) {
	...
	HandleCudaError(cudaMemcpyToArray(m_volumeLutArray, 0, 0, img->channel(channel)->m_lut, LUT_SIZE * 4, cudaMemcpyHostToDevice));
}
