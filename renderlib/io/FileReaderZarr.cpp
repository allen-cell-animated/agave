...
FileReaderZarr::loadFromFile(const LoadSpec& loadSpec)
{
  ...
  for (uint32_t channel = 0; channel < nch; ++channel) {
    ...
    if (levelDims.dtype == "uint8") {
      auto arr = tensorstore::Array<uint8_t, 5>(reinterpret_cast<uint8_t*>(destptr), shapeToLoad, tensorstore::c_order);
      tensorstore::Read(m_store | transform, tensorstore::UnownedToShared(arr)).value();
    } else if (levelDims.dtype == "int32") {
      auto arr = tensorstore::Array<int32_t, 5>(reinterpret_cast<int32_t*>(destptr), shapeToLoad, tensorstore::c_order);
      tensorstore::Read(m_store | transform, tensorstore::UnownedToShared(arr)).value();
    } else if (levelDims.dtype == "uint16") {
      auto arr =
        tensorstore::Array<uint16_t, 5>(reinterpret_cast<uint16_t*>(destptr), shapeToLoad, tensorstore::c_order);
      tensorstore::Read(m_store | transform, tensorstore::UnownedToShared(arr)).value();
    } else if (levelDims.dtype == "float32") {
      auto arr = tensorstore::Array<float, 5>(reinterpret_cast<float*>(destptr), shapeToLoad, tensorstore::c_order);
      tensorstore::Read(m_store | transform, tensorstore::UnownedToShared(arr)).value();
    } else {
