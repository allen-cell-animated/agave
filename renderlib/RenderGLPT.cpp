...
void RenderGLPT::doRender(const CCamera& camera) {
    ...
    // Render image
	//RayMarchVolume(_cudaF32Buffer, _volumeTex, _volumeGradientTex, m_renderSettings, _w, _h, 2.0f, 20.0f, glm::value_ptr(m), _channelMin, _channelMax);
//	cudaFB theCudaFB = {
//		_cudaF32Buffer,
//		_cudaF32AccumBuffer,
//		_randomSeeds1,
...
//		_timingRender, _timingBlur, _timingPostProcess, _timingDenoise, numIterations);
	...
	//m_renderSettings->m_DenoiseParams.m_LerpC = 0.33f * (max((float)m_renderSettings->GetNoIterations(), 1.0f) * 1.0f);//1.0f - powf(1.0f / (float)gScene.GetNoIterations(), 15.0f);//1.0f - expf(-0.01f * (float)gScene.GetNoIterations());
...
//	LOG_DEBUG << "Window " << _w << " " << _h << " Cam " << m_renderSettings->m_Camera.m_Film.m_Resolution.GetResX() << " " << m_renderSettings->m_Camera.m_Film.m_Resolution.GetResY();
