...
void CameraController::OnMouseMove(QMouseEvent *event) 
{
	...
	if (event->buttons() & Qt::LeftButton)
	{
		if (GetShiftKey() && GetCtrlKey())
		{
			...
			m_qcamera->GetFocus().SetFocalDistance(std::max(0.0f, m_CCamera->m_Focus.m_FocalDistance + m_ApertureSpeed * (float)(m_NewPos[1] - m_OldPos[1])));
		...
		else
		{
			if (GetShiftKey())
			{
				...
				m_qcamera->GetAperture().SetSize(std::max(0.0f, m_CCamera->m_Aperture.m_Size + m_ApertureSpeed * (float)(m_NewPos[1] - m_OldPos[1])));
			...
			else if (GetCtrlKey())
			{
				...
				m_qcamera->GetProjection().SetFieldOfView(std::max(0.0f, m_CCamera->m_FovV - m_FovSpeed * (float)(m_NewPos[1] - m_OldPos[1])));
			...
			else
			{
				...
				m_CCamera->Orbit(-0.6f * m_OrbitSpeed * (float)(m_NewPos[1] - m_OldPos[1]), -m_OrbitSpeed * (float)(m_NewPos[0] - m_OldPos[0]));
