...
Util::compileShader(QByteArray computeShaderSource)
{
...
QOpenGLShaderProgram*
Util::compileShader(QByteArray vertexShaderSource, QByteArray fragmentShaderSource)
{
  return Util::compileShader(vertexShaderSource, QByteArray(), fragmentShaderSource);
}

QOpenGLShaderProgram*
Util::compileShader(QByteArray vertexShaderSource, QByteArray geometryShaderSource, QByteArray fragmentShaderSource)
{
  return Util::compileShader(
    vertexShaderSource, QByteArray(), QByteArray(), geometryShaderSource, fragmentShaderSource);
}

QOpenGLShaderProgram*
Util::compileShader(QByteArray vertexShaderSource,
                    QByteArray tcsShaderSource,
                    QByteArray tesShaderSource,
                    ...
                    QByteArray fragmentShaderSource)
{
  ...
  if (!vShader->compileSourceCode(vertexShaderSource)) {
    LOG_DEBUG << vShader->log().toStdString();
  }
  if (tcs)
  ...
  if (!fShader->compileSourceCode(fragmentShaderSource)) {
    LOG_DEBUG << fShader->log().toStdString();
  }

  QOpenGLShaderProgram* shader;
  shader = new QOpenGLShaderProgram();
  if (!shader->addShader(vShader)) {
    LOG_DEBUG << shader->log().toStdString();
  }
  if (tcs)
  ...
  if (ok) {
    ...
    LOG_DEBUG << "Shader not linked:";
    LOG_DEBUG << QString(fragmentShaderSource).toStdString();
    LOG_DEBUG << shader->log().toStdString();
  }
  shader->bind();
...
QByteArray
Util::loadSource(QString fileNameOrSource, bool loadFromDisk)
{
...
QOpenGLShaderProgram*
Util::loadAndCompileShader(QString computeFileName)
{
  QByteArray computeSourceArray;
  computeSourceArray = Util::loadSource(computeFileName, true);

  LOG_DEBUG << "compiling" << computeFileName.toStdString() << " ...";
...
QOpenGLShaderProgram*
Util::loadAndCompileShader(QString vsFileName,
                           QString gsFileName,
                           QString fsFileName,
                           bool loadVS,
                           bool loadGS,
...
QOpenGLShaderProgram*
Util::loadAndCompileShader(QString vsFileName, QString fsFileName, bool loadVS, bool loadFS)
{
  return Util::loadAndCompileShader(vsFileName, "", fsFileName, loadVS, false, loadFS);
}

// QOpenGLShaderProgram *Util::loadAndCompileShader(QString vsFileName, QString gsFileName, QString fsFileName, bool
// loadVS, bool loadGS, bool loadFS)
QOpenGLShaderProgram*
Util::loadAndCompileShader(QString vsFileName,
                           QString tcsFileName,
                           QString tesFileName,
                           ...
                           bool loadFS)
{
  ...
  LOG_DEBUG << "compiling" << vsFileName.toStdString() << " + " << fsFileName.toStdString() << "...";
...
qreal
Util::mix(qreal a0, qreal a1, qreal x)
{
...
qreal
Util::mix(qreal a0, qreal a1, qreal a2, qreal a3, qreal x)
{
  qreal p, q, r, s;
  p = (a3 - a2) - (a0 - a1);
  q = (a0 - a1) - p;
...
QPointF
Util::mixPointF(QPointF p0, QPointF p1, qreal x)
{
  return QPointF(mix(p0.x(), p1.x(), x), mix(p0.y(), p1.y(), x));
}

QPointF
Util::mixPointF(QPointF p0, QPointF p1, QPointF p2, QPointF p3, qreal x)
{
  return QPointF(mix(p0.x(), p1.x(), p2.x(), p3.x(), x), mix(p0.y(), p1.y(), p2.y(), p3.y(), x));
}

QPointF
Util::mixPointFCos(QPointF p0, QPointF p1, qreal x)
{
  return QPointF(mix(p0.x(), p1.x(), x), mixCos(p0.y(), p1.y(), x));
}

QVector2D
Util::mixVector2D(QVector2D p0, QVector2D p1, qreal x)
{
  return QVector2D(mix(p0.x(), p1.x(), x), mix(p0.y(), p1.y(), x));
}

QVector2D
Util::mixVector2D(QVector2D p0, QVector2D p1, QVector2D p2, QVector2D p3, qreal x)
{
  return QVector2D(mix(p0.x(), p1.x(), p2.x(), p3.x(), x), mix(p0.y(), p1.y(), p2.y(), p3.y(), x));
}

QVector3D
Util::mixVector3D(QVector3D p0, QVector3D p1, qreal x)
{
  return QVector3D(mix(p0.x(), p1.x(), x), mix(p0.y(), p1.y(), x), mix(p0.z(), p1.z(), x));
}

QVector3D
Util::mixVector3D(QVector3D p0, QVector3D p1, QVector3D p2, QVector3D p3, qreal x)
{
  return QVector3D(mix(p0.x(), p1.x(), p2.x(), p3.x(), x),
